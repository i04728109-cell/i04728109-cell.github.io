...
Дано: [variable] = [value] [unit]
Решение: [formula] → [substitution] → [calculation]
...
TYPE D — CHEMISTRY:
Дано: [substances]
Решение: [reaction + molar steps]
Ответ: [result]

TYPE E — GENERAL:
Short clear answer. Ответ: [summary]

RULE 5 — NO long explanations.
RULE 6 — Each step on its OWN line.
RULE 7 — IMAGES: detect type and solve correctly.`;
...
function chatTitle(msgs) {
  ...
  return f.text.length > 38 ? f.text.slice(0, 38) + "…" : f.text;
}
  ...
  return (
    <div style={{ fontSize: "0.93rem", lineHeight: 1.85, wordBreak: "break-word" }}>
      {lines.map((line, i) => {
        ...
        if (isResh) return <div key={i} style={{ marginTop: "0.75rem", fontWeight: 700, fontSize: "0.95rem", color: dark ? "#e8e6f8" : "#12121e", paddingBottom: "0.3rem", borderBottom: `2px solid ${b2}` }}>{t}</div>;
        if (isMetod) return <div key={i} style={{ marginTop: "0.4rem", marginBottom: "0.2rem", fontSize: "0.82rem", fontStyle: "italic", color: dark ? "#7070a0" : "#8080a0", padding: "0.25rem 0.7rem", background: dark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.03)", borderRadius: 6, borderLeft: "2px solid #e11d48" }}>{t}</div>;
        if (isPust) return <div key={i} style={{ fontStyle: "italic", color: dark ? "#9090c0" : "#5050a0", marginTop: "0.2rem" }}>{t}</div>;
...
function SidebarContent({ dark, sideBord, txtMut, txtPri, chats, groupedChats, activeId, setActiveId, setMobileOpen, newChat, setDelConfirm }) {
  const d = dark;
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      {/* header */}
      <div style={{ padding: "0.9rem 1rem 0.75rem", borderBottom: `1px solid ${sideBord}`, display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        <Logo size={28} />
      ...
      <div style={{ padding: "0.75rem 0.75rem 0.4rem", flexShrink: 0 }}>
        <button onClick={newChat} style={{ width: "100%", background: "transparent", border: `1px dashed ${d ? "#2a2a3e" : "#c8c8e0"}`, borderRadius: 10, padding: "0.55rem 1rem", display: "flex", alignItems: "center", gap: 8, cursor: "pointer", color: txtMut, fontSize: "0.84rem", fontFamily: "'Inter',sans-serif", transition: "all .2s" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 0.5rem 1rem" }}>
        {chats.length === 0 && (
          <div style={{ textAlign: "center", color: txtMut, fontSize: "0.78rem", marginTop: "2rem", padding: "0 1rem", lineHeight: 1.6 }}>
            Нет чатов.<br />Начни разговор!
          </div>
        )}
        {Object.entries(groupedChats).map(([grp, list]) => (
          <div key={grp}>
            <div style={{ fontSize: "0.67rem", color: txtMut, fontWeight: 600, letterSpacing: "0.06em", padding: "0.75rem 0.6rem 0.25rem", textTransform: "uppercase" }}>{grp}</div>
            {list.map(chat => (
              <div
                key={chat.id}
                onClick={() => { setActiveId(chat.id); setMobileOpen(false); }}
                style={{ display: "flex", alignItems: "center", gap: 8, padding: "0.5rem 0.65rem", borderRadius: 9, cursor: "pointer", transition: "all .15s", borderLeft: chat.id === activeId ? "2px solid #e11d48" : "2px solid transparent", marginBottom: 2, background: chat.id === activeId ? (d ? "rgba(225,29,72,0.1)" : "rgba(225,29,72,0.07)") : "transparent" }}
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={txtMut} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: .6 }}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
                ...
                <button
                  ...
                  style={{ background: "transparent", border: "none", cursor: "pointer", color: txtMut, opacity: .35, flexShrink: 0, display: "flex", alignItems: "center", padding: 2, transition: "all .15s", borderRadius: 4 }}
                >
      ...
      {/* footer */}
      <div style={{ borderTop: `1px solid ${sideBord}`, padding: "0.6rem 1rem", display: "flex", justifyContent: "center", flexShrink: 0 }}>
        <span style={{ fontSize: "0.64rem", color: "#e11d48", fontWeight: 600, letterSpacing: "0.08em" }}>by ismail</span>
      </div>
    </div>
  );
}
...
export default function SPLINTAI() {
  ...
  const sideBord = d ? "#1a1a2c" : "#dcdcec";
  ...
  const clearImg = () => { setImgB64(null); setImgType(null); setPreview(null); if (fileRef.current) fileRef.current.value = ""; };
  ...
  const send = async () => {
    ...
    if (loading) return;
    let chatId = activeId;
    if (!chatId) { chatId = uid(); setChats(p => [{ id: chatId, messages: [], createdAt: Date.now() }, ...p]); setActiveId(chatId); }
  ...
  const groupedChats = chats.reduce((a, c) => {
    ...
    const g = cd.toDateString() === td.toDateString() ? "Сегодня" : cd.toDateString() === yd.toDateString() ? "Вчера" : cd.toLocaleDateString("ru-RU", { day: "numeric", month: "long" });
    if (!a[g]) a[g] = [];
    a[g].push(c);
    return a;
  }, {});

  const sidebarProps = { dark: d, sideBord, txtMut, txtPri, chats, groupedChats, activeId, setActiveId, setMobileOpen, newChat, setDelConfirm };
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .sbtn:hover{opacity:.7}
      `}</style>

      <div style={{ display: "flex", width: "100vw", height: "100vh", background: bg, color: txtPri, fontFamily: "'Inter',sans-serif" }}>

        {/* ── SIDEBAR always visible on desktop ── */}
        <div style={{ width: 256, minWidth: 256, maxWidth: 256, background: sideBg, borderRight: `1px solid ${sideBord}`, height: "100vh", overflow: "hidden", display: "flex", flexDirection: "column",
          /* hide on mobile */
        ...
        {mobileOpen && (
          <div style={{ position: "fixed", inset: 0, zIndex: 60 }}>
            <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,.55)" }} onClick={() => setMobileOpen(false)} />
            <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 256, background: sideBg, borderRight: `1px solid ${sideBord}`, zIndex: 1 }}>
              <SidebarContent {...sidebarProps} />
            </div>
          </div>
        )}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          <div style={{ background: surf, borderBottom: `1px solid ${bord}`, padding: "0.75rem 1.25rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {/* mobile menu */}
              <button className="sbtn" onClick={() => setMobileOpen(v => !v)} style={{ background: "transparent", border: `1px solid ${bord}`, borderRadius: 9, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: txtMut, transition: "all .2s" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></svg>
              </button>
              <Logo size={28} />
              <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "1.05rem", letterSpacing: "-.02em" }}>SPLINT<span style={{ color: "#e11d48" }}>.</span>AI</span>
            </div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              {pwaPrompt && (
                <button onClick={async () => { pwaPrompt.prompt(); const { outcome } = await pwaPrompt.userChoice; if (outcome === "accepted") setPwaPrompt(null); }} style={{ background: "#e11d48", border: "none", borderRadius: 8, padding: "0.35rem 0.75rem", fontSize: "0.75rem", color: "white", cursor: "pointer", fontFamily: "'Inter',sans-serif", fontWeight: 600 }}>Установить</button>
              )}
              <button className="sbtn" onClick={newChat} style={{ background: "transparent", border: `1px solid ${bord}`, borderRadius: 8, padding: "0.35rem 0.75rem", fontSize: "0.75rem", color: txtMut, cursor: "pointer", fontFamily: "'Inter',sans-serif", display: "flex", alignItems: "center", gap: 5, transition: "all .2s" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
                Новый чат
              </button>
              <button className="sbtn" onClick={() => setDark(!d)} style={{ background: "transparent", border: `1px solid ${bord}`, borderRadius: 9, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: txtMut, transition: "all .2s" }}>
                {d ? (
            ...
            <div style={{ maxWidth: 680, margin: "0 auto" }}>
              {activeMsgs.length === 0 && (
                <div style={{ textAlign: "center", padding: "3rem 0 2rem", animation: "fadeUp .5s ease" }}>
                  ...
                  <h1 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(2rem,8vw,3.5rem)", letterSpacing: "-.03em", marginBottom: "0.5rem" }}>SPLINT<span style={{ color: "#e11d48" }}>.</span>AI</h1>
                  <p style={{ color: txtMut, fontSize: "0.88rem", lineHeight: 1.6 }}>Задай любой вопрос или скинь фото задачи<br />Любой предмет · Любой язык</p>
                </div>
              )}
          ...
          {/* input */}
          <div style={{ background: surf, borderTop: `1px solid ${bord}`, padding: "0.85rem 1rem 1rem", flexShrink: 0 }} onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); loadImg(e.dataTransfer.files[0]); }}>
            <div style={{ maxWidth: 680, margin: "0 auto" }}>
              {preview && (
              ...
              {dragging && <div style={{ border: "2px dashed #e11d48", borderRadius: 12, padding: "0.75rem", textAlign: "center", fontSize: "0.8rem", color: "#e11d48", marginBottom: "0.6rem", background: d ? "#0d0d1e" : "#fff0f3" }}>Отпусти фото сюда</div>}
              <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                <button className="sbtn" onClick={() => fileRef.current?.click()} style={{ background: "transparent", border: `1px solid ${bord}`, borderRadius: 10, width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                ...
                <textarea ref={taRef} style={{ flex: 1, background: inpBg, border: `1px solid ${dragging ? "#e11d48" : bord}`, borderRadius: 12, color: txtPri, fontSize: "0.92rem", padding: "0.65rem 0.9rem", resize: "none", outline: "none", lineHeight: 1.6, height: 40, minHeight: 40, maxHeight: 140, transition: "border .2s", overflow: "hidden" }} placeholder="Напиши вопрос или задачу..." value={input} onChange={e => { setInput(e.target.value); e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 140) + "px"; }} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} />
                <button className="send-btn" onClick={send} disabled={loading || (!input.trim() && !imgB64)} style={{ background: (loading || (!input.trim() && !imgB64)) ? (d ? "#1a1a28" : "#e8e8f0") : "#e11d48", border: "none", borderRadius: 10, width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", cursor: (loading || (!input.trim() && !imgB64)) ? "not-allowed" : "pointer", flexShrink: 0, transition: "all .2s", color: (loading || (!input.trim() && !imgB64)) ? txtMut : "white" }}>
      ...
      {delConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: surf, border: `1px solid ${bord}`, borderRadius: 16, padding: "1.5rem", maxWidth: 300, width: "90%", textAlign: "center", animation: "fadeUp .2s ease" }}>
            <div style={{ fontSize: "1.5rem", marginBottom: "0.75rem" }}>🗑️</div>
            <div style={{ fontWeight: 600, marginBottom: "0.4rem", color: txtPri }}>Удалить чат?</div>
            <div style={{ fontSize: "0.82rem", color: txtMut, marginBottom: "1.25rem" }}>Это действие нельзя отменить</div>
